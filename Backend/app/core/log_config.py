...
# 使用项目结构信息从当前文件定位项目根目录
# 当前文件位于 app/core/log_config.py，向上两级（而非三级）到达项目根目录
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(script_path), "../.."))

# 移除调试输出，避免在多进程环境中重复打印
    ...
    if (worker_id is not None and worker_id == "0") or worker_id is None:
        try:
            ...
            if not os.path.exists(MASTER_PROCESS_FILE):
                ...
                # 尝试确认进程是否存在
                try:
                    os.kill(int(master_pid), 0)  # 不发送信号，仅检查进程是否存在
                    # 进程存在，不是主进程
...
class LogQueueProcessor(threading.Thread):
    ...
    def stop(self):
        self.running = False
        self.join(timeout=2)  # 最多等待 2 秒让线程完成
...
class SafeTimedRotatingFileHandler(logging.handlers.TimedRotatingFileHandler):
    ...
    def __init__(self, filename, when='h', interval=1, backupCount=0, encoding=None, delay=False, utc=False, atTime=None):
        ...
        # 避免多进程环境下的文件权限冲突
        self.delay = True  # 延迟文件创建，直到第一条日志写入
        self.mode = 'a'    # 始终使用追加模式
